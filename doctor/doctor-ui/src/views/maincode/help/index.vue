  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="类别" prop="category">
        <el-select v-model="queryParams.category" placeholder="请选择类别" clearable>
          <el-option
            v-for="dict in dict.type.help_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['maincode:help:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['maincode:help:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="helpList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类别" align="center" prop="category">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.help_type" :value="scope.row.category"/>
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.help_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="访问次数" align="center" prop="visitCount" />
      <el-table-column label="点赞数" align="center" prop="likeCount" />
      <el-table-column label="相关链接" align="center" prop="relatedLinks" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="相关链接" prop="relatedLinks">
          <el-input v-model="form.relatedLinks" type="textarea" placeholder="请输入内容" />
        </el-form-item>
...
<script>
import { listHelp, getHelp, delHelp, addHelp, updateHelp } from "@/api/maincode/help";

export default {
  name: "Help",
  dicts: ['help_type', 'help_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除帮助内容编号为"' + helpIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('maincode/help/export', {
        ...this.queryParams
